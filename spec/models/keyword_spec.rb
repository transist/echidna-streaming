...
describe Keyword do
  ...
  context "#save" do
    ...
    it "should increment count for minute interval key" do
      expect($redis.zrange @keyword.second_interval_key, 0, -1, with_scores: true).to be_include ["中国", 1.0]
    ...
    it "should set source_id" do
      expect($redis.get @keyword.second_source_id_key).to eq "source-1"
  ...
  context "keys" do
    ...
    its(:second_source_id_key) { should eq "groups/group-1/second/1361465734/中国/source_id" }
    ...
    its(:second_keywords_key) { should eq "groups/group-1/second/keywords" }
